...
##    colors = ['b4ecb4'] * 24
    ...
    hourchart = CreateDayHourChart (start, data)
    ...
    averagechart = CreateAverageChart (averages)
    ...
    items = [['Average kW Usage/Day (today)', 'SELECT avg(POWER) FROM TED_history_hour WHERE DATE >= %s AND DATE < %s', [start, end]], 
                ['Average kW Usage/Day (last 7 days)', 'SELECT avg(POWER) FROM TED_history_day WHERE DATE >= %s AND DATE < %s', [end - datetime.timedelta(days=7), end]],
                ['Average kW Usage/Day (last 14 days)', 'SELECT avg(POWER) FROM TED_history_day WHERE DATE >= %s AND DATE < %s', [end - datetime.timedelta(days=14), end]],
                ['Average kW Usage/Day (last 28 days)', 'SELECT avg(POWER) FROM TED_history_day WHERE DATE >= %s AND DATE < %s', [end - datetime.timedelta(days=28), end]],
                ['Maximum kW Usage/Day (today)', 'SELECT max(POWER) FROM TED_history_hour WHERE DATE >= %s AND DATE < %s', [start, end]], 
                ['Minimum kW Usage/Day (today)', 'SELECT min(POWER) FROM TED_history_hour WHERE DATE >= %s AND DATE < %s', [start, end]],
                ['Total kW Usage (today)', 'SELECT sum(POWER) FROM TED_history_hour WHERE DATE >= %s AND DATE < %s', [start, end]],
                ]
    ...
    for key, query, daterange in items:
        ...
        if x:
            statistics[key] = round(x,2)

    return render_to_response ('electric/bar_day.html', {'hourchart': hourchart, 'date' : start, 'budgetchart': budgetchart, 'averagechart': averagechart, 'statistics' : statistics}, context_instance=RequestContext(request))
...
def bar_week(request, year, week):
    ...
    weekchart = CreateWeekDayChart (data)
    ...
    return render_to_response ('electric/bar_week.html', {'weekchart':weekchart, 'date' : weekrequest, 'budgetchart' : budgetchart, 'scatterchart' : scatter}, context_instance=RequestContext(request)) # 'data' : data, 'date' : weekrequest, 'title' : title, 'colors' : string.join(colors, '|'), 'budgetchart' : budgetchart}, context_instance=RequestContext(request))
...
def bar_month(request, year, month):
    ...
    monthchart = CreateMonthDayChart (data)
...
def bills(request):
    ...
    for item in list(history):
        ...
        data.append ({'DATE_START' : item.DATE_START,
                      ...
                      'DAYS' : days,
                      'WATTDAY' : avg_watt_per_day,
                      'COSTDAY' : avg_cost_per_day, })


    return render_to_response ('electric/bills.html', {'bills': data,}, context_instance=RequestContext(request))
